...
export default function Page(){
  ...
  async function run(e:React.FormEvent){
    ...
    try{
      ...
      setOut(await api.postForm("/api/v1/llm/feedback/resume-score", fd));
  ...
  return (
    <form onSubmit={run} className="space-y-3">
      ...
      <button disabled={!file||busy} className="px-4 py-2 rounded-xl bg-slate-900 text-white">{busy?"Scoring…":"Score Resume"}</button>
